...
class UserProfileUpdate(BaseModel):
    ...
    NOTE: superseded by ProfileUpdate below for the unified
    PUT /profile/me endpoint. Kept here in case anything else
    in the codebase still references it — safe to delete once
...
class UserProfileOut(BaseModel):
    ...
    FIX APPLIED: added driver_profile below so GET /profile/me
    returns both passenger and driver info in one payload.
    """
    ...
    # -----------------------------------------------------------
    # FIX: nested driver profile.
    # None until the user has ever touched a driver field/photo.
    # -----------------------------------------------------------
...
class DriverProfileOut(BaseModel):
    ...
    FIX APPLIED: chattiness/music/smoking/pets retyped from
    Optional[str] to their real Enum types, matching the
    SQLAlchemy Enum columns on DriverProfile. Optional[str]
    would fail (or serialize wrong) against actual enum
    instances coming from the ORM.
    """
...
# ===============================================================
# COMBINED PROFILE UPDATE (PASSENGER + DRIVER)
# NEW — used by PUT /profile/me in the unified profile router
# ===============================================================

class ProfileUpdate(BaseModel):
    """
    Single input schema for updating both passenger and driver
    fields in one request. All fields optional — only the ones
    the caller actually sends get applied (the router uses
    exclude_unset=True and splits fields by name into passenger
    vs. driver updates).
    """

    # --- passenger fields (mirrors UserProfileUpdate) -----------
    full_name: Optional[str] = None
    ...
    # --- driver fields (mirrors DriverProfileUpdate) ------------
    license_number: Optional[str] = None
